...
public final class ZkCloudname extends Thread implements Cloudname, Watcher {
    ...
    public void run() {
        ...
        while (true) {
            // First sleep a bit, and then check if we should exit.
            try {
                Thread.sleep(1000);
            ...
            if (state == ZooKeeper.States.CONNECTING) {
                ...
                if (connectingCounter > 10) {
                    log.fine("Long time in connecting, forcing a close of zookeeper,.");
            ...
            if (state == ZooKeeper.States.CLOSED) {
                ...
                try {
                    zkObjectHandler.setZooKeeper(new ZooKeeper(connectString, SESSION_TIMEOUT, this));
                } catch (IOException e) {
                    log.log(Level.SEVERE, "RetryConnection failed for some reason:" + e.getMessage(), e);
                }
                try {
                    // Wait a bit so the next reconnection does not happen to
                    // frequently in case of problems.
                    Thread.sleep(10000);
                } catch (InterruptedException e) {
                    return;
                }
    ...
    public ZkCloudname connectWithTimeout(long waitTime, TimeUnit waitUnit) throws CloudnameException {
        ...
        try {
            zkObjectHandler = new ZkObjectHandler(new ZooKeeper(connectString, SESSION_TIMEOUT, this));
    ...
    public ServiceHandle claim(Coordinate coordinate) {
        ...
        ClaimedCoordinate statusAndEndpoints = new ClaimedCoordinate(coordinate, zkObjectHandler.getClient());
        ...
        ZkServiceHandle handle = new ZkServiceHandle(statusAndEndpoints, coordinate, zkObjectHandler.getClient());
    ...
    public ServiceStatus getStatus(Coordinate coordinate) throws CloudnameException {
        String statusPath = ZkCoordinatePath.getStatusPath(coordinate);
        ZkCoordinateData zkCoordinateData = ZkCoordinateData.loadCoordinateData(statusPath, zkObjectHandler.getClient().getZookeeper(), null);
    ...
    public String getConfig(final Coordinate coordinate) throws CoordinateMissingException, CloudnameException {
        ...
        Stat stat = new Stat();
        try {
            byte[] data = zkObjectHandler.getClient().getZookeeper().getData(configPath, false, stat);
            if (data == null) {
                return null;
            }
    ...
    public void listRecursively(List<String> nodeList) throws CloudnameException, InterruptedException {
        Util.listRecursively(zkObjectHandler.getClient().getZookeeper(), ZkCoordinatePath.getCloudnameRoot(), nodeList);
    }
